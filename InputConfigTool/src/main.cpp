...
void run()
{
	...
	try {
		SDL::Surface icon;
		icon.load("data/InputConfigTool/icon.png");
		window.setIcon(icon);
	}
	catch (...) {}

	ImGuiContextWrapper imguiContextWrapper{ window, glContext };
	imguiContextWrapper.addFont("data/InputConfigTool/font.ttf", 14.0f);
	...
	try {
		backgroundTexture = SDL::Surface::loadTexture("data/InputConfigTool/control.png");
	}
	catch (...) {}
