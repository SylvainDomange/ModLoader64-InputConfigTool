...
{
	if (boost::algorithm::starts_with(str, "Right")) *v = JoyHatDir::Right;
	else if (boost::algorithm::starts_with(str, "Left")) *v = JoyHatDir::Left;
	else if (boost::algorithm::starts_with(str, "Down")) *v = JoyHatDir::Down;
	else if (boost::algorithm::starts_with(str, "Up")) *v = JoyHatDir::Up;
	else return false;
	return true;
}
...
Map parseSingleMap(const std::string& str)
{
	...
	if (boost::algorithm::starts_with(str, "key") && std::sscanf(cstr, "key(%d)", &i[0]) == 1) {
		map.type = MapType::Key;
		map.key.scancode = static_cast<SDL_Scancode>(sdl_keysym2native(i[0]));
	}
	else if (boost::algorithm::starts_with(str, "button") && std::sscanf(cstr, "button(%d)", &i[0]) == 1) {
		map.type = MapType::JoyButton;
		map.jbutton.button = i[0];
	}
	else if (boost::algorithm::starts_with(str, "hat") && std::sscanf(cstr, "hat(%d %10s", &i[0], buf.data()) == 2) {
		if (hatDirFromStr(buf, &map.jhat.dir)) {
			map.type = MapType::JoyHat;
			map.jhat.hat = i[0];
		}
	}
	else if (boost::algorithm::starts_with(str, "axis")) {
...
std::pair<Map, Map> parseDoubleMap(const std::string& str)
{
	...
	if (boost::algorithm::starts_with(str, "key") && std::sscanf(cstr, "key(%d,%d)", &i[0], &i[1]) == 2) {
	...
	else if (boost::algorithm::starts_with(str, "button") && std::sscanf(cstr, "button(%d,%d)", &i[0], &i[1]) == 2) {
		map.first.type = map.second.type = MapType::JoyButton;
		map.first.jbutton.button = i[0];
		map.second.jbutton.button = i[1];
	}
	else if (boost::algorithm::starts_with(str, "hat") && std::sscanf(cstr, "hat(%d %10s %10s", &i[0], buf[0].data(), buf[1].data()) == 3) {
	...
	else if (boost::algorithm::starts_with(str, "axis") && std::sscanf(cstr, "axis(%d%c,%d%c)", &i[0], &j[0], &i[1], &j[1]) == 4) {
