...
bool hatDirFromStr(std::string_view str, JoyHatDir* v)
{
	if (str.substr(0, 5) == "Right") *v = JoyHatDir::Right;
	else if (str.substr(0, 4) == "Left") *v = JoyHatDir::Left;
	else if (str.substr(0, 4) == "Down") *v = JoyHatDir::Down;
	else if (str.substr(0, 2) == "Up") *v = JoyHatDir::Up;
	else return false;
	return true;
}
...
Map parseSingleMap(const std::string& str)
{
	...
	if (view.substr(0, 3) == "key" && std::sscanf(cstr, "key(%d)", &i[0]) == 1) {
		map.type = MapType::Key;
		map.key.scancode = static_cast<SDL_Scancode>(sdl_keysym2native(i[0]));
	}
	else if (view.substr(0, 6) == "button" && std::sscanf(cstr, "button(%d)", &i[0]) == 1) {
		map.type = MapType::JoyButton;
		map.jbutton.button = i[0];
	}
	else if (view.substr(0, 3) == "hat" && std::sscanf(cstr, "hat(%d %10s", &i[0], buf.data()) == 2) {
		if (hatDirFromStr(buf, &map.jhat.dir)) {
			map.type = MapType::JoyHat;
			map.jhat.hat = i[0];
		}
	}
	else if (view.substr(0, 4) == "axis") {
...
std::pair<Map, Map> parseDoubleMap(const std::string& str)
{
	...
	if (str.substr(0, 3) == "key" && std::sscanf(cstr, "key(%d,%d)", &i[0], &i[1]) == 2) {
	...
	else if (str.substr(0, 6) == "button" && std::sscanf(cstr, "button(%d,%d)", &i[0], &i[1]) == 2) {
		map.first.type = map.second.type = MapType::JoyButton;
		map.first.jbutton.button = i[0];
		map.second.jbutton.button = i[1];
	}
	else if (str.substr(0, 3) == "hat" && std::sscanf(cstr, "hat(%d %10s %10s", &i[0], buf[0].data(), buf[1].data()) == 3) {
	...
	else if (str.substr(0, 4) == "axis" && std::sscanf(cstr, "axis(%d%c,%d%c)", &i[0], &j[0], &i[1], &j[1]) == 4) {
